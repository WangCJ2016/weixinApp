! function(n, e) {
    ...
    n.run(["$templateCache", function(n) { n.put("ionic-citypicker.html",
    	'<div ng-class="{\'city-picker-backdrop\' : vm.backdrop }"><ion-modal-view class=city-picker-bg><div class=city-bgclose ng-click=vm.clickToClose()></div><div class=city-picker><header class="bar city-picker-header" ng-class=vm.barCssClass><div class=buttons><button class="button button-clear button-positive" ng-click=vm.returnCancel()>取消</button></div><h1 class=title></h1><div class=buttons><button class="button button-clear button-positive" ng-click=vm.returnOk()>{{vm.buttonText}}</button></div></header><div class="city-picker-inner has-header"><div class=row><div class="col col-33"><ion-scroll-minh delegate-handle={{vm.provinceHandle}} on-scroll-complete="vm.getValue(\'province\')" scrollbar-y=false><ul><li ng-repeat="province in vm.AreaService">{{province.name}}</li></ul></ion-scroll-minh></div><div class="col col-33"><ion-scroll-minh delegate-handle={{vm.cityHandle}} on-scroll-complete="vm.getValue(\'city\')" scrollbar-y=false><ul><li ng-repeat="city in vm.province.sub">{{city.name}}</li></ul></ion-scroll-minh></div><div class="col col-33"><ion-scroll-minh delegate-handle={{vm.countryHandle}} on-scroll-complete="vm.getValue(\'country\')" scrollbar-y=false><ul><li ng-repeat="country in vm.city.sub">{{country.name}}</li></ul></ion-scroll-minh></div></div><div class=picker-center-highlight></div></div></div></ion-modal-view></div>') }]) }(), angular.module("ionic-citypicker.directive", ["ionic"]).directive("ionScrollMinh", ["$timeout", "$controller", "$ionicBind", "$ionicConfig", function(n, e, a, m) {
...
app.directive("ionicCityPicker", ["$ionicPopup", "$timeout", "CityPickerService", "$ionicScrollDelegate", "$ionicModal", function(n, e, a, m, s) {
    ...
    return { restrict: "AE", template: "<div class={{vm.cssClass}}><i class={{vm.iconClass}}></i>{{vm.title}}<span class=item-note>{{vm.areaData}}</span></div></div>", scope: { options: "=options" }, link: function(n, u, b) {
            var o = n.vm = {},
                ...
                o.barCssClass = angular.isDefined(r.barCssClass) ? r.barCssClass : "bar-stable", o.backdrop = t(r.backdrop) ? r.backdrop : !0,
                ...
                o.AreaService = a,
                o.tag = r.tag || "-",
                o.step = r.step || 50, angular.isDefined(r.defaultAreaData) && r.defaultAreaData.length > 1 ? (o.defaultAreaData = r.defaultAreaData,
                o.areaData = o.defaultAreaData.join(o.tag)) : (o.defaultAreaData = ["北京", "东城区"],
                o.areaData = angular.isDefined(r.areaData) ? r.areaData.join(o.tag) : "请选择城市"),
                r.areaData = o.areaData.split(o.tag),
                o.returnOk = function() { o.city && o.city.sub && o.city.sub.length > 0 ? o.areaData = o.province.name + o.tag + o.city.name + o.tag + o.country.name : o.areaData = o.province.name + o.tag + o.city.name,
                r.areaData = o.areaData.split(o.tag), e(function() { c && c.hide(), r.buttonClicked && r.buttonClicked() }, 50) },
                o.returnCancel = function() {
                	c && c.hide(),
                	e(function() { o.initAreaData(o.areaData.split(o.tag)) }, 150) },
                o.clickToClose = function() { o.backdropClickToClose && o.returnCancel() },
